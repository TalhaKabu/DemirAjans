...
public class CartDal(ISqlDataAccess db) : ICartDal
{
    ...
    public async Task InsertAsync(CartDto cartDto) => await _db.SaveDataAsync(storedProcedure: "spCarts_Insert", new { cartDto.UserId, cartDto.ColorId, cartDto.Quantity, cartDto.CreationDate, cartDto.LastModificationDate });
}