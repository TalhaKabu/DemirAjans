...
public class ColorDal(ISqlDataAccess db) : IColorDal
{
    ...
    public async Task InsertAsync(ColorDto colorDto) => await _db.SaveDataAsync(storedProcedure: "spColors_Insert", new { colorDto.Name, colorDto.Code, colorDto.ProductId, colorDto.Uid, colorDto.ImageName, colorDto.LastModificationDate, colorDto.CreationDate });
}